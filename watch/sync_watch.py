...
sync_watch.py - pipeline 1 lenh cho module Watch:
  ...
  3. Sync vao watch/watchData.js (chi viet vung AUTO-SYNC, giu entry tay/Youtube)
  4. ID = ten file video (khong duoi) de de mapping.

Dung:
    python watch/sync_watch.py                      # quet + transcribe + sync
    python watch/sync_watch.py --sync-only           # chi sync .srt co san -> data.js
    python watch/sync_watch.py --force               # lam lai tu dau
    python watch/sync_watch.py --model small --lang en
...
DEFAULT_DATA = os.path.join(HERE, 'watchData.js')
VIDEO_EXTS = ('.mp4', '.mkv', '.mov', '.webm')
AUDIO_EXTS = ('.mp3', '.wav', '.m4a', '.ogg', '.oga', '.aac', '.flac')
MEDIA_EXTS = VIDEO_EXTS + AUDIO_EXTS
START_MARK = '/* AUTO-SYNC-START - do not edit: generated by watch/sync_watch.py */'
...
def entry_js(video_file, segments_vi):
    stem = os.path.splitext(video_file)[0]
    lines = ['\t{',
             '\t\tid: %s,' % js_str(stem),
             '\t\ttitle: %s,' % js_str(stem_title(stem)),
             "\t\ttype: 'file',",
             '\t\tsrc: %s,' % js_str('watch/media/' + video_file),
             '\t\tsubs: [']
...
def sync_data(data_js, video_files):
    ...
    if START_MARK not in content or END_MARK not in content:
        ...
        idx = content.rfind('];')
        if idx < 0:
            sys.exit('❌ %s khong co mang WATCH_DATA.' % data_js)
    ...
    for vf in sorted(video_files):
        ...
        for s, e, t in segs_en:
            merged.append((s, e, t + ' ||| ' + vi_by_en.get((s, e), '')))
        if not merged:
